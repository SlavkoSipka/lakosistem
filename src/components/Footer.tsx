...
interface FooterProps {
  onNavigate?: (page: 'home' | 'papirna' | 'hemija' | 'prevoz' | 'kontakt' | 'usluge' | 'o-projektu') => void;
}
...
export function Footer({ onNavigate }: FooterProps = {}) {
  ...
  const handleOProjektuClick = (e: React.MouseEvent) => {
    if (onNavigate) {
      e.preventDefault();
      onNavigate('o-projektu');
    }
  };
        ...
        <div className="border-t border-gray-800 pt-6 md:pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4">
              <button
                onClick={handleOProjektuClick}
                className="text-gray-500 hover:text-[#00A65C] transition-colors text-xs md:text-sm border border-gray-700 px-3 py-1.5 rounded-md hover:border-[#00A65C] focus:outline-none focus:ring-2 focus:ring-[#00A65C]/50"
                aria-label="O projektu - saznajte kako je rađen sajt"
              >
                {t('footer.case_study')}
              </button>
